...
def run_exercicio_111(menu_window):
    ...
    def tx_callback(params):
        ...
        if modo == "Paridade Par":
            bits_para_transmitir = enlace.encode_paridade(bits_para_transmitir)
        elif modo == "Checksum":
            bits_para_transmitir = enlace.encode_checksum(bits_para_transmitir)
        elif modo == "CRC-32":
            bits_para_transmitir = enlace.encode_crc(bits_para_transmitir)
        ...
        # 5. Decode de detecção de erro
        if modo == "Paridade Par":
            payload, erro = enlace.decode_paridade(bits_desenquadrados)
        elif modo == "Checksum":
            payload, erro = enlace.decode_checksum(bits_desenquadrados)
        elif modo == "CRC-32":
            payload, erro = enlace.decode_crc(bits_desenquadrados)
...
def run_exercicio_112(menu_window):
    ...
    def tx_callback(params):
        ...
        if modulation == "ASK":
            ...
            while len(bits_para_transmitir) % 4 != 0:
                bits_para_transmitir.append(0)
        ...
        if modo == "Paridade Par":
            if len(bits_desenquadrados) < 1:
                payload, erro = [], True
            else:
                payload, erro = enlace.decode_paridade(bits_desenquadrados)
        elif modo == "Checksum":
            if len(bits_desenquadrados) < 3:
                payload, erro = [], True
            else:
                payload, erro = enlace.decode_checksum(bits_desenquadrados)
        elif modo == "CRC-32":
            if len(bits_desenquadrados) < 32:
                payload, erro = [], True
            else:
                payload, erro = enlace.decode_crc(bits_desenquadrados)
